...
# Usage: curl -fsSL https://raw.githubusercontent.com/Graviton28/server-monitoring/main/loki-grafana/install.sh | bash
set -euo pipefail

REPO_URL="https://github.com/Graviton28/server-monitoring.git"
INSTALL_DIR="${INSTALL_DIR:-$HOME/server-monitoring}"
APP_DIR="$INSTALL_DIR/loki-grafana"
...
Install complete: $APP_DIR

Next steps:
  1. Edit $APP_DIR/.env with your Loki URL and auth.
  2. Run it:
       cd $APP_DIR && npm run agent
     Or keep it running persistently with pm2:
       npm install -g pm2
       pm2 start $APP_DIR/src/agent/index.js --name server-monitor
       pm2 save
